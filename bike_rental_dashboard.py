...
sns.scatterplot(data=main_df, x='temp', y='cnt', alpha=0.6, ax=ax8)
ax8.set_title('Hubungan antara Suhu dan Jumlah Penyewaan Sepeda')
ax8.set_xlabel('Suhu (temp)')
ax8.set_ylabel('Jumlah Penyewaan (cnt)')
ax8.grid(True)
st.pyplot(fig8)
st.write('Penyewaan paling tinggi ditunjukkan pada hari jumat sabtu dan minggu dibandingkan hari kerja.')

# Visualisasi pertanyaan bisnis 2
st.subheader('Total Rentals per Weekday')
weekday_counts = all_df.groupby('weekday')['cnt'].sum().reset_index()

# Map weekday labels (0 = Monday, 6 = Sunday)
weekday_labels = {0: 'Senin', 1: 'Selasa', 2: 'Rabu', 3: 'Kamis', 4: 'Jumat', 5: 'Sabtu', 6: 'Minggu'}
weekday_counts['weekday'] = weekday_counts['weekday'].map(weekday_labels)

fig9, ax9 = plt.subplots()
sns.barplot(data=weekday_counts, x='weekday', y='cnt', palette='coolwarm', ax=ax9)
ax9.set_title('Distribusi Jumlah Penyewaan Sepeda Berdasarkan Hari dalam Seminggu')
ax9.set_xlabel('Hari dalam Minggu')
ax9.set_ylabel('Jumlah Penyewaan (cnt)')
ax9.grid(axis='y')
st.pyplot(fig9)
...
workingday_df.groupby(working_labels)['cnt'].sum().plot(kind='bar', ax=ax3, color=['green'])
